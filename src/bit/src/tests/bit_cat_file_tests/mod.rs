...
init"#;
    bit!("-C src/tests/repos/foo cat-file -p HEAD").stdout(expected);
}

#[test]
fn test_cli_cat_file_pretty_commit_output() {
    bit!("-C src/tests/repos/foo cat-file -p bc01ca0eb625f386c396605d7813be3e522df1c0")
        .stdout("qux content\n");
}

#[test]
fn test_cli_cat_file_partial_oid() {
    bit!("-C src/tests/repos/foo cat-file -p bc01ca0").stdout("qux content\n");
}
