...
impl Arbitrary for MutableTree {
    fn arbitrary(g: &mut Gen) -> Self {
...
#[quickcheck]
fn serialize_then_parse_tree(tree: MutableTree) -> BitResult<()> {
    let mut bytes = vec![];
    tree.serialize(&mut bytes)?;
    let parsed = MutableTree::deserialize_sized_unbuffered(bytes.as_slice(), bytes.len() as u64)?;
...
fn parse_then_serialize_tree() -> BitResult<()> {
    ...
    let tree = MutableTree::deserialize_sized_unbuffered(bytes, bytes.len() as u64)?;
